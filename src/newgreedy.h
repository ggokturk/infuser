...
#if defined __AVX512F__ && !defined NOAVX512
...
void get_s_cc(unsigned* s_cc, const graph_t& g, const vector<unsigned>& S, const size_t k, const int R, uint32_t labels_ptr[], float counts_ptr[], const int BLOCKSIZE) {
	if (k < 2) return;
	for (int r = 0; r < R; r += BLOCKSIZE) {
		float* counts = &(counts_ptr[r * g.n]);
		uint32_t* labels = &(labels_ptr[r * g.n]);
...
#pragma omp parallel for
	for (int r = 0; r < R; r += BLOCKSIZE)
	{
		...
		bool cont = true;
		auto active = get_aligned<bool>(g.n), nactive = get_aligned<bool>(g.n);
		fill(active.get(), active.get() + g.n, true);
		fill(nactive.get(), nactive.get() + g.n, false);
		while (true) {
			//cont = false;
			//#pragma omp parallel for schedule(dynamic)
			for (int n = 0; n < g.n; n++) {
...
void newgreedy2(const graph_t& g, const int K, const int R, const size_t blocksize) {
	...
	sort(rand_seeds.get(), rand_seeds.get() + R);
	...
	while (S.size() < K) {
		...
		if (iteration[u] == S.size()) {
			...
			//q.pop()
			//q.push(u);
			int elements = min(q.size(), level1);
